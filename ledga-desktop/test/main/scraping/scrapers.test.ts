...
describe("ScrapingManager with fixture emails", () => {
    it("scrapes Wema credit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "wema_credit.eml"), "utf-8")
    ...
    it("scrapes Wema debit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "wema_debit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
    ...
    it("scrapes Wema debit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "vat_wema.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
    ...
    it("scrapes Wema debit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "comm_wema.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
    ...
    it("scrapes GTBank credit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "gtbank_credit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
        ...
        expect(result).toMatchObject({
            type: "credit",
            account_number: "******2362",
            merchant: "000017260222155905030369722585-ALAT NIP TRANSFER TO BASSEY BJORN DONALD EFFIOM F",
            merchant_account: null,
            bank: "GTBank",
    ...
    it("scrapes GTBank debit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "gtbank_debit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
    ...
    it("scrapes EcoBank credit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "ecobank_credit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
    ...
    it("scrapes First Bank debit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "firstbank_debit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
    ...
    it("scrapes Zenith Bank debit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "zenith_debit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
    ...
    it("scrapes RenMoney debit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "renmoney_debit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
        ...
        expect(result).toMatchObject({
            ...
            merchant: "You just sent some money from your Renmoney account to Cecil Akpan.",
            merchant_account: null,
            bank: "RenMoney",
            bank_reference: "",
    ...
    it("scrapes Access Bank credit email and returns expected transaction", async () => {
        const rawEml = fs.readFileSync(path.join(FIXTURES_DIR, "access_credit.eml"), "utf-8")
        const manager = createScrapingManager()
        const result = await manager.scrape(rawEml)
