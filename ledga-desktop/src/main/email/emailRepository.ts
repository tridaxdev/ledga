...
export class EmailRepository {
  ...
  findByConnectionAndEmailId(
    connectionId: string,
    emailId: string,
  ): EmailRow | null {
    const sql =
      "SELECT * FROM emails WHERE connection_id = ? AND email_id = ? LIMIT 1";
    const rows = this.db.executeQuery(sql, [connectionId, emailId]) as
      | EmailRow[]
      | unknown;
  ...
  updateMetadata(
    ...
    const sql =
      "UPDATE emails SET from_addr = ?, timestamp = ?, content_hash = ? WHERE id = ?";
  ...
  findIdsByConnectionAndStatus(
    ...
    const rows = this.db.executeQuery(sql, [connectionId, ...statuses]) as
      | { id: string }[]
      | unknown;
    const list = Array.isArray(rows) ? rows : [];
  ...
  findIdsByStatus(statuses: EmailStatus[]): string[] {
    ...
    const rows = this.db.executeQuery(sql, statuses) as
      | { id: string }[]
      | unknown;
    const list = Array.isArray(rows) ? rows : [];
    return list.map((r) => r.id);
  ...
  getMaxTimestampForConnection(connectionId: string): number | null {
    const sql =
      "SELECT MAX(timestamp) as max_ts FROM emails WHERE connection_id = ?";
    const rows = this.db.executeQuery(sql, [connectionId]) as
      | { max_ts: number | null }[]
      | unknown;
    const list = Array.isArray(rows) ? rows : [];
  ...
  getProcessedCountToday(): number {
    ...
    const rows = this.db.executeQuery(
      "SELECT COUNT(*) as count FROM emails WHERE status = ? AND timestamp >= ?",
      ["processed", startOfTodayUnixSec],
    ) as { count: number }[] | unknown;
    return Array.isArray(rows) ? Number(rows[0]?.count ?? 0) : 0;
  }

  getProcessingCounts(): { processing: number; failed: number } {
    const processingRows = this.db.executeQuery(
      "SELECT COUNT(*) as count FROM emails WHERE status IN (?, ?)",
      ["pending", "processing"],
    ) as { count: number }[] | unknown;
    const failedRows = this.db.executeQuery(
      "SELECT COUNT(*) as count FROM emails WHERE status = ?",
      ["failed"],
    ) as { count: number }[] | unknown;
    const processing = Array.isArray(processingRows)
      ? Number(processingRows[0]?.count ?? 0)
      : 0;
    const failed = Array.isArray(failedRows)
      ? Number(failedRows[0]?.count ?? 0)
      : 0;
