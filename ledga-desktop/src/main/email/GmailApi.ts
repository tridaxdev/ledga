...
import type { EmailApi, EmailMessageHeader, EmailMessageListItem } from "./EmailApi"
import { ALLOWED_BANK_SENDERS } from "./bankSenders"
import { GMAIL_MESSAGES_LIST_MAX_TOTAL, GMAIL_MESSAGES_LIST_PAGE_SIZE } from "./constants"
...
export type RefreshTokensFn = (connection: ConnectionWithTokens) => Promise<ConnectionWithTokens>
...
export class GmailApi implements EmailApi {
    ...
    private async ensureValidToken(): Promise<void> {
        ...
        const expired = !this.connection.expiryDate || this.connection.expiryDate.getTime() - marginMs < Date.now()
        if (expired) {
            this.logger.warn("Gmail API: access token expired or expiring, refreshing")
    ...
    private async request<T>(
        ...
        if (res.status === 401) {
            ...
            if (options.body !== undefined) {
                ;(retryInit.headers as Record<string, string>)["Content-Type"] = "application/json"
    ...
    async listMessages(startDate: Date, endDate: Date): Promise<EmailMessageListItem[]> {
        const after = Math.floor(startDate.getTime() / 1000)
        const before = Math.floor(endDate.getTime() / 1000)
        const fromQuery = ALLOWED_BANK_SENDERS.map(addr => `from:${addr}`).join(" OR ")
        ...
        do {
            ...
            for (const m of batch) {
                ...
                if (collected.length >= GMAIL_MESSAGES_LIST_MAX_TOTAL) {
                    return collected.slice(0, GMAIL_MESSAGES_LIST_MAX_TOTAL).map(m => ({ id: m.id }))
                }
            }
    ...
    async getMessageMetadata(providerMessageId: string): Promise<EmailMessageHeader | null> {
        try {
            const msg = await this.request<GmailMessageMetadataResponse>(`/messages/${providerMessageId}`, {
                query: { format: "full" }
            })
            const fromHeader = msg.payload?.headers?.find((h: { name: string; value: string }) => h.name.toLowerCase() === "from")
            const fromAddr = fromHeader?.value?.trim() ?? ""
            const match = fromAddr.match(/<([^>]+)>/)
            const fromEmail = match ? match[1].trim().toLowerCase() : fromAddr.trim().toLowerCase()
            if (!ALLOWED_BANK_SENDERS.some(a => fromEmail.includes(a.toLowerCase()))) {
                this.logger.debug("Gmail API: getMessageMetadata skipped, From not in allowed list", { providerMessageId, fromEmail, fromAddr: fromAddr.slice(0, 80) })
                return null
            }

            const dateHeader = msg.payload?.headers?.find((h: { name: string; value: string }) => h.name.toLowerCase() === "date")
    ...
    async getMessageContent(providerMessageId: string): Promise<string> {
        ...
        const msg = await this.request<GmailMessageRawResponse>(`/messages/${providerMessageId}`, { query: { format: "raw" } })
        if (!msg.raw) {
