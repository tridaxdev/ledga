...
export function setupIpcHandlersForEmail(
  ...
  registerIpcHandler(AllowedChannelIpc.EmailsWaitFor, async (_, ...args) => {
    ...
    if (request.connectionId) {
      ...
      return ResultFactory.from(
        emailService.waitForConnectionEmails(request.connectionId),
      );
    }
    ...
    return ResultFactory.error(
      new Error("emails:wait-for requires connectionId or emailIds"),
    );
  });
