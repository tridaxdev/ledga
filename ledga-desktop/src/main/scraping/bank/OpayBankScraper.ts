...
function parseOpayDate(value: string): string {
    ...
    const match = value.match(/^(\w+)\s+(\d{1,2})(?:st|nd|rd|th)?,\s+(\d{4})\s+(\d{1,2}):(\d{2}):(\d{2})$/i)
    if (!match) return ""
...
function extractOpayDetail(html: string, label: string): string {
    ...
    const re = new RegExp(`>\\s*${escaped}\\s*</span>[\\s\\S]*?left_em[\\s\\S]*?<span>([^<]*)</span`, "i")
...
export class OpayBankScraper extends BankScraper {
    ...
    detect(parsed: ParsedEmail): EmailTransactionType | null {
        ...
        if (subj.includes("transfer successful") || subj.includes("payment successful")) return "debit"
        return null
    }
    ...
    parse(parsed: ParsedEmail, type: EmailTransactionType): NormalizedTransaction {
        ...
        if (isTransfer) {
            ...
            const balanceMatch = html.match(/available\s+balance\s+is[\s\S]*?(?:₦|NGN)\s*([\d,]+\.?\d*)/i)
            const available_balance = balanceMatch ? parseOpayAmount(balanceMatch[1].trim()) : 0
        ...
        const amountMatch = html.match(/payment\s+of[\s\S]*?(?:₦|NGN)\s*([\d,]+\.?\d*)/i)
