...
function extractTableValue(html: string, label: string): string {
  ...
  const re = new RegExp(
    `${labelEsc}\\s*<\\/td>[\\s\\S]*?<td[^>]*>\\s*:\\s*<\\/td>\\s*<td[^>]*>([\\s\\S]*?)<\\/td>`,
    "i",
  );
...
export class GTBankScraper extends BankScraper {
  ...
  parse(
    ...
    const rawAmount = amountCell
      ? parseGTBankAmount(amountCell.replace(/^NGN\s*/i, ""))
      : 0;
    const amount = Math.abs(rawAmount);

    const valueDateMatch = html.match(
      /Value\s+Date[\s\S]*?(\d{4}-\d{2}-\d{2})/i,
    );
    const valueDate = valueDateMatch ? valueDateMatch[1] : "";

    const timeMatch = html.match(
      /Time\s+of\s+Transaction[\s\S]*?(\d{1,2}:\d{2}:\d{2}\s*[AP]M)/i,
    );
    const time24 = timeMatch
      ? parseGTBankTimeTo24h(timeMatch[1].trim())
      : "00:00:00";
    const timestamp = valueDate && time24 ? `${valueDate}T${time24}` : "";

    const bank_reference = extractTableValue(html, "Document Number");

    const availableBalanceMatch = html.match(
      /Available\s+Balance[\s\S]*?NGN\s*(?:&nbsp;)?\s*(-?\s*[\d,]+\.?\d*)/i,
    );
    const available_balance = availableBalanceMatch
      ? parseGTBankAmount(availableBalanceMatch[1])
      : 0;
