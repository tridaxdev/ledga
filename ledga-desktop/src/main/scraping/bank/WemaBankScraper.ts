...
export class WemaBankScraper extends BankScraper {
  ...
  parse(
    ...
    const accountNumberMatch = html.match(
      /Account\s+Number[\s\S]*?color:\s*#7F7F7F[^>]*>([^<]+)<[\s\S]*?color:\s*#7F7F7F[^>]*>([^<]+)/i,
    );
    const account_number = accountNumberMatch
      ? accountNumberMatch[2].replace(/\s+/g, " ").trim()
      : "";

    // Table has label cell, colon cell (#7F7F7F), value cell (#7F7F7F); capture second occurrence
    const descriptionMatch = html.match(
      /Description[\s\S]*?color:\s*#7F7F7F[^>]*>([^<]+)<[\s\S]*?color:\s*#7F7F7F[^>]*>([^<]+)/i,
    );
    const merchant = descriptionMatch
      ? descriptionMatch[2]
          .replace(/\s+/g, " ")
          .replace("VAT ALAT NIP TRANSFER TO ", "")
          .replace("COMM ALAT NIP TRANSFER TO ", "")
          .trim()
      : "";

    const referenceMatch = html.match(
      /Reference\s+Number[\s\S]*?color:\s*#7F7F7F[^>]*>([^<]+)<[\s\S]*?color:\s*#7F7F7F[^>]*>([^<]+)/i,
    );
    const bank_reference = referenceMatch
      ? referenceMatch[2].replace(/\s+/g, " ").trim()
      : "";

    const amountMatch = html.match(
      /Transaction\s+Amount[\s\S]*?([\d,]+\.?\d*)\s*NGN/i,
    );
    const rawAmount = amountMatch
      ? parseWemaAmount(`${amountMatch[1]} NGN`)
      : 0;
    const amount = Math.abs(rawAmount);

    const dateTimeMatch = html.match(
      /Transaction\s+Date\s*&(?:amp;)?\s*Time[\s\S]*?color:\s*#7F7F7F[^>]*>([^<]+)<[\s\S]*?color:\s*#7F7F7F[^>]*>(\d{2}-\d{2}-\d{4}\s+\d{2}:\d{2}:\d{2})/i,
    );
    const timestamp = dateTimeMatch ? parseWemaTimestamp(dateTimeMatch[2]) : "";

    const balanceMatch = html.match(
      /Current\s+Balance\s+as\s+at[\s\S]*?([\d,]+\.\.?\d+)\s*NGN/i,
    );
    const available_balance = balanceMatch
      ? parseWemaAmount(`${balanceMatch[1].replace(/\.\./g, ".")} NGN`)
      : 0;

    const currency = WemaBankScraper.extractCurrency(
      html,
      parsed.subject ?? "",
    );
