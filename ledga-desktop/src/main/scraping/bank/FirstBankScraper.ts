...
function isFirstBankEmail(parsed: ParsedEmail): boolean {
    ...
    return /FirstAlert@firstbanknigeria\.com/i.test(html) && /FirstBank\s+Alert/i.test(html)
}
...
function extractBoldLabelValue(html: string, label: string): string {
    ...
    const re = new RegExp(`<p[^>]*font-weight:\\s*bold[^>]*>\\s*${labelPattern}\\s*</p>[\\s\\S]*?<p[^>]*>([^<]*)</p>`, "i")
...
function parseForwardedHeaderDate(html: string): string {
    const re = /Date:\s*\w+,\s*(\w+)\s+(\d{1,2}),\s*(\d{4})\s+at\s+(\d{1,2}):(\d{2})\s*(AM|PM)/i
    const m = html.match(re)
    if (!m) return ""
...
function parseBodyDateTime(raw: string): string {
    const m = raw.match(/(\d{1,2})-(\w{3})-(\d{2,4})\s+(\d{1,2}):(\d{2})\s*(AM|PM)/i)
    if (!m) return raw
...
export class FirstBankScraper extends BankScraper {
    ...
    parse(parsed: ParsedEmail, type: EmailTransactionType): NormalizedTransaction {
        ...
        const timestamp = parseForwardedHeaderDate(html) || parseBodyDateTime(extractBoldLabelValue(html, "Date/Time"))

        const currency = FirstBankScraper.extractCurrency(html, parsed.subject ?? "")
