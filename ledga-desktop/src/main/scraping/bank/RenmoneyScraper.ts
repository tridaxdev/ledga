...
export class RenmoneyScraper extends BankScraper {
    ...
    parse(parsed: ParsedEmail, type: EmailTransactionType): NormalizedTransaction {
        ...
        const merchantMatch = text.match(/Hi\s+\w+,\s*\n\n([\s\S]+?)\.\s*Please\s+see\s+details\s+below/i)
        const merchant = merchantMatch ? `${merchantMatch[1].replace(/\s+/g, " ").trim()}.` : ""
        ...
        const amountMatch = text.match(/(?:Debit|Credit)\s+Amount:?\s*\n?\s*[\u20a6]?([\d,]+\.?\d*)/i)
        const absAmount = amountMatch ? parseRenmoneyAmount(amountMatch[1]) : 0
        const amount = Math.abs(absAmount)

        const balanceMatch = text.match(/current\s+balance[\s\S]*?[\u20a6]\s*([\d,]+\.?\d*)/i)
        const available_balance = balanceMatch ? parseRenmoneyAmount(balanceMatch[1]) : 0

        const currency = RenmoneyScraper.extractCurrency(parsed.html ?? "", parsed.subject ?? "")
