            ...
            >
                ...
                >
                    ...
                    <button
                        ...
                        style={{ color: 'var(--color-ledga-text-muted)', cursor: 'pointer', border: 'none', background: 'transparent', display: 'inline-flex' }}
                    >
                ...
                <div style={{ padding: '24px 22px' }}>
                    ...
                    {step === 'mailbox' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
                            ...
                            <div style={{ fontSize: '13px', color: 'var(--color-ledga-text-muted)' }}>
                                You'll choose your Google account in the next step, and grant Ledga
                    ...
                    {step === 'waiting' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '13px', alignItems: 'center', textAlign: 'center', padding: '8px 0' }}>
                            <Spinner />
                            <p style={{ margin: 0, color: 'var(--color-ledga-text-secondary)', fontSize: '14px' }}>
                    ...
                    {step === 'sync-preference' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
                            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '24px', fontWeight: 600, margin: 0, color: 'var(--color-ledga-text)' }}>
                                Keep Ledga up to date?
                            </h2>
                            {connectedEmail && (
                                <div style={{ fontSize: '13px', color: 'var(--color-ledga-text-muted)' }}>{connectedEmail}</div>
                            )}
                            <SyncOption
                                selected={autoSync}
                                onSelect={() => setAutoSync(true)}
                                title="Automatic sync"
                                description="Ledga is notified the moment a bank email arrives — no polling."
                            />
                            <SyncOption
                                selected={!autoSync}
                                onSelect={() => setAutoSync(false)}
                                title="Manual only"
                                description="Sync when you click the button."
                            />
    ...
    return (
        ...
        >
            ...
            <span>
                ...
                <span style={{ display: 'block', fontSize: '13px', color: 'var(--color-ledga-text-secondary)', marginTop: '2px', lineHeight: 1.5 }}>
                    {description}
                </span>
            </span>
        </button>
