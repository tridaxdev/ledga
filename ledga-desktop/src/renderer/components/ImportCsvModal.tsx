                ...
                <div style={{ padding: 22 }}>
                    {step === 'drop' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            ...
                            <button onClick={browseFile} style={{ alignSelf: 'flex-start', background: 'var(--color-ledga-brand)', color: '#fff', border: 'none', borderRadius: 7, padding: '10px 18px', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
                                Browse files
                            </button>
                    ...
                    {step === 'importing' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            ...
                            <div style={{ fontSize: 18, fontWeight: 600, fontFamily: 'var(--font-serif)', color: 'var(--color-ledga-text)' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, color: 'var(--color-ledga-text-secondary)' }}>
                                <span>{rowsParsed} / {totalRows || '…'} rows parsed</span>
                            ...
                            <div style={{ display: 'flex', gap: 9 }}>
                                <button onClick={handleFinish} style={{ background: 'var(--color-ledga-brand)', color: '#fff', border: 'none', borderRadius: 7, padding: '10px 18px', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
                                    Go to ledger
                                </button>
                                <button
                                    onClick={() => { onViewStatus(); handleClose() }}
                                    style={{ border: '1px solid var(--color-ledga-border)', background: '#fff', borderRadius: 7, padding: '9px 16px', fontSize: 14, fontWeight: 500, color: 'var(--color-ledga-text)', cursor: 'pointer' }}
                                >
                                    View status
                                </button>
                            </div>
