...
function AssistantScreen() {
    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <div style={{ flexShrink: 0, padding: '16px 28px', borderBottom: '1px solid var(--color-ledga-border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--color-ledga-bg)' }}>
                <div>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: '1px solid var(--color-ledga-brand-border)', background: 'var(--color-ledga-brand-bg)', borderRadius: 999, padding: '4px 11px', fontSize: 12, color: 'var(--color-ledga-brand)', fontWeight: 500 }}>
                    <LedgerIcon />
                ...
                <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    {isThinking && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-start', maxWidth: '92%' }}>
                            <div style={{ background: '#fff', border: '1px solid var(--color-ledga-border)', borderRadius: '14px 14px 14px 4px', padding: '10px 14px', fontSize: 14, color: 'var(--color-ledga-text-muted)' }}>
                                Thinking
                                <span style={{ display: 'inline-block', width: 2, height: '1em', background: 'var(--color-ledga-text-muted)', marginLeft: 3, verticalAlign: 'text-bottom', animation: 'blink .7s step-start infinite' }} />
                            </div>
                        </div>
                    )}
                    ...
                    {isStreaming && streamingText && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-start', maxWidth: '92%' }}>
                            <div style={{ background: '#fff', border: '1px solid var(--color-ledga-border)', borderRadius: '14px 14px 14px 4px', padding: '11px 14px', fontSize: 14, lineHeight: 1.6, color: 'var(--color-ledga-text)', whiteSpace: 'pre-wrap' }}>
                                {streamingText}
                            </div>
                        </div>
                    )}
                    ...
                    {error && (
                        <div style={{ fontSize: 13, color: 'var(--color-ledga-danger)' }}>{error}</div>
                    )}
            ...
            <div style={{ flexShrink: 0, padding: '14px 28px 18px', borderTop: '1px solid var(--color-ledga-border-subtle)', background: 'var(--color-ledga-bg)' }}>
                <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', alignItems: 'flex-end', gap: 10, background: '#fff', border: '1px solid var(--color-ledga-border)', borderRadius: 12, padding: '8px 8px 8px 14px', boxShadow: '0 1px 2px rgba(63,56,47,.05)' }}>
                    <textarea
                        value={input}
                        ...
                        rows={1}
                        style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'inherit', fontSize: 14, color: 'var(--color-ledga-text)', padding: '8px 0', resize: 'none', maxHeight: 120 }}
                    />
                    {isStreaming ? (
    ...
    if (message.role === 'user') {
        return (
            <div style={{ display: 'flex' }}>
                <div style={{ alignSelf: 'flex-end', marginLeft: 'auto', background: 'var(--color-ledga-brand)', color: '#fff', borderRadius: '14px 14px 4px 14px', padding: '10px 14px', fontSize: 14, lineHeight: 1.55, maxWidth: '84%', whiteSpace: 'pre-wrap' }}>
                    {message.content}
                </div>
            </div>
        )
...
function summarizeToolOutput(output: unknown): string {
    if (output && typeof output === 'object' && 'error' in output) {
        return `error: ${(output as { error: unknown }).error}`
    }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
            <button
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: 8, border: '1px solid var(--color-ledga-border)', background: 'var(--color-ledga-sidebar)', borderRadius: 999, padding: '5px 12px', cursor: 'pointer', maxWidth: '100%', width: 'fit-content' }}
            >
                <SearchIcon />
            ...
            {open && (
                <div style={{ border: '1px solid var(--color-ledga-border)', borderLeft: '2px solid #6db78c', background: '#fff', borderRadius: 8, padding: '11px 13px', display: 'flex', flexDirection: 'column', gap: 9 }}>
                    {toolCalls.map(tc => (
                        <div key={tc.toolCallId} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            ...
                            <div style={{ fontSize: 12, color: 'var(--color-ledga-text-secondary)', fontFamily: 'monospace', background: 'var(--color-ledga-sidebar)', border: '1px solid var(--color-ledga-border-subtle)', borderRadius: 5, padding: '4px 8px', overflowWrap: 'break-word' }}>
